...
public class BruteforceParallel
{
    ...
    public static int Bruteforce(byte[] data, byte[] hash)
    {
        ...
        for (var i = 0; i < threadCount; i++)
        {
            ...
            var start = i * sizeof(byte);
            var end = start + chunkSize * sizeof(byte);

            if (i == threadCount - 1)
                end += overChunk * sizeof(byte);
            
            var thread = new Thread(() => Bruteforce(threadBin, hash, start, end));
            thread.Start();
            threads.Add(thread);
        }
    ...
    /// <returns></returns>
    public static void Bruteforce(byte[] data, byte[] hash, int from, int to)
    {
